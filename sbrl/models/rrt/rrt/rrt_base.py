...
from sbrl.models.rrt.rrt.tree import Tree
from sbrl.models.rrt.utilities.geometry import steer
...
class RRTBase(object):
    def __init__(self, X, Q, x_init, x_goal, max_samples, r, prc=0.01, distance_fn=None):
        ...
        self.distance_fn = distance_fn  # if this is not none, we will use a hand coded nearest fn (linear search)
    ...
    def nearby(self, tree, x, n):
        ...
        if self.distance_fn is None:
            ...
            tr = self.trees[tree]
            hits = list(tr.intersection(self.X.dimension_lengths.T.flatten(), object=True))  # (N, 2) rolled out [N.., N..]
            for
