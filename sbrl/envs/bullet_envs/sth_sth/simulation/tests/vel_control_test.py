...
if __name__ == '__main__':
    ...
    params.robot_controller_mode = RCM.xdot_pid  # vel
    params.robot_controller_params = d(type=CT.PID, k_p=1., dim=6, difference_fn=lambda a, b: a - b)  # velocity difference
    ...
    pose = np.zeros(6)
    delta = 0.1 * np.sin(W * np.arange(T))

    waypoints = np.stack([list(pose) + [0.]] * T)
    waypoints[:, 2] += delta  # z

    logger.info("Starting ctrl.")
    for i in range(waypoints.shape[0]):
        observation, _, done = env.step(waypoints[i])
        pose = np.concatenate([observation.ee_velocity[0], observation.ee_angular_velocity[0]])
